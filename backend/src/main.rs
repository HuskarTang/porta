...
async fn main() {
    let app = Router::new()
        ...
        .route("/porta/node/config", post(ok))
        .route("/porta/node/key/import", post(ok))
        .route("/porta/node/key/generate", post(ok))
        ...
        .route("/porta/community/service/list", get(get_community_services))
        .route("/porta/community/node/ban", post(ok))
        .route("/porta/community/node/unban", post(ok))
        .route("/porta/community/node/list", get(get_community_nodes))
...
async fn get_node_info() -> Json<Value> {
    Json(json!({
        "name": "我的节点",
...
async fn get_communities() -> Json<Value> {
    Json(json!([
        {
            "id": "dev-community",
            "name": "开发者社区",
...
async fn get_discovered_services() -> Json<Value> {
    Json(json!([
        {
            "uuid": "svc-jupyter",
            "name": "Jupyter Hub",
...
async fn get_subscribed_services() -> Json<Value> {
    Json(json!([
        {
            "id": "sub-web",
            "name": "Web Dashboard",
...
async fn get_sessions() -> Json<Value> {
    Json(json!([
...
async fn get_access_url() -> Json<Value> {
    Json(json!({ "local_url": "http://localhost:8080" }))
}

async fn get_published_services() -> Json<Value> {
    Json(json!([
        {
            "id": "pub-dashboard",
            "name": "Team Dashboard",
...
async fn get_community_nodes() -> Json<Value> {
    Json(json!([
...
async fn get_community_services() -> Json<Value> {
    Json(json!([
        { "id": "svc-web", "name": "Web 服务器", "uuid": "a1b2c3d4...", "protocol": "HTTP", "port": 8080, "online": true, "announced": true },
        { "id": "svc-api", "name": "API 网关", "uuid": "b2c3d4e5...", "protocol": "HTTPS", "port": 443, "online": true, "announced": true },
        { "id": "svc-db", "name": "数据库服务", "uuid": "c3d4e5f6...", "protocol": "TCP", "port": 5432, "online": false, "announced": false },
        { "id": "svc-ssh", "name": "SSH 隧道", "uuid": "d4e5f6a7...", "protocol": "SSH", "port": 22, "online": true, "announced": false }
    ]))
}

async fn get_proxy_status() -> Json<Value> {
    Json(json!({
        "enabled": true,
        "listen_port": 1080
    }))
